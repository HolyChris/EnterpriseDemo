...
.run(function($http) {
  ...
  .controller('NewSiteCtrl', function ($scope,$location,$http,ENV,Flash,$modal,Managers) {
  ...
  $scope.addManager = function($item, $model, $label) {
    ...
    manage_ids.push('&manage_ids[]='+$item.id);
    $scope.managersSelected = undefined; // clear input
  }
  $scope.removeManager = function(id) {
    manage_ids.splice(manage_ids.indexOf('&manage_ids[]='+id), 1);
